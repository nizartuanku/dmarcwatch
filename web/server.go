...
import (
	...
	"github.com/nizartuanku/dmarcwatch/verify"
)
...
func (s *Server) handleAddTarget(w http.ResponseWriter, r *http.Request) {
	...
	if !canAdd(s.effLimits(), current) {
		httpError(w, http.StatusPaymentRequired,
			"target limit reached for your tier — Pro and Team: https://whop.com/nizar-tuanku/dmarcwatch?utm_source=app")
		return
	}
...
func (s *Server) handleScanNow(w http.ResponseWriter, r *http.Request) {
	if !s.effLimits().ScanNow {
		httpError(w, http.StatusPaymentRequired,
			"on-demand scans are a Pro feature — scheduled scans continue as normal. Pro and Team: https://whop.com/nizar-tuanku/dmarcwatch?utm_source=app")
		return
	}
